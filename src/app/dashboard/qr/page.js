...
// Helper to rotate canvas image by 180 degrees
...
// Dynamic TTF Font downloader & virtual file-system register for jsPDF
...
// Custom Premium QR drawing engine utilizing HTML5 Canvas and node-qrcode's raw bitmatrix
async function drawCustomQR(text, options) {
  ...
  const margin = 28; // border padding
  const drawSize = canvasSize - (margin * 2);
  const cellSize = drawSize / size;
  
  // Checker to see if a module is part of the 3 large corner Finder Patterns
  ...
  for (let r = 0; r < size; r++) {
    for (let c = 0; c < size; c++) {
      if (qr.modules.data[r * size + c]) {
        // Skip finder eyes (rendered with custom structures later)
        if (isEyeModule(r, c)) continue;
        
        // Skip central blocks if logo/monogram overlay is enabled
        if (options.showLogo) {
        ...
        if (options.dotStyle === 'circle') {
          // Circular dot pattern style
          const radius = (cellSize / 2) * 0.85;
          ctx.arc(x + cellSize / 2, y + cellSize / 2, radius, 0, Math.PI * 2);
          ctx.fill();
        } else {
          // Classic crisp squares
  ...
  // Custom Finder Eye drawing utility
  const drawEye = (x, y) => {
    ...
    // Draw inner eye dots (3x3 modules)
    ctx.beginPath();
  ...
  if (options.showLogo) {
    ...
    // Draw backing card shape to clear QR module overlap
    ctx.fillStyle = options.bgColor || '#ffffff';
    ctx.beginPath();
    ...
    if (options.logoUrl) {
      ...
      if (logoImg.complete && logoImg.naturalWidth > 0) {
        ...
        // Fallback Monogram initials inside backing circle
      ...
      // Draw Monogram initials as fallback center card
      ctx.fillStyle = options.fgColor;
      ctx.font = `bold ${Math.floor(logoSize * 0.65)}px sans-serif`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
...
export default function QRPage() {
  ...
  const [qrStyleType, setQrStyleType] = useState('solid'); // 'solid' | 'gradient'
  const [gradientDirection, setGradientDirection] = useState('diagonal'); // 'diagonal' | 'horizontal' | 'vertical' | 'radial'
  const [dotStyle, setDotStyle] = useState('square'); // 'square' | 'circle'
  const [eyeStyle, setEyeStyle] = useState('square'); // 'square' | 'rounded' | 'circle'
  const [eyeColor, setEyeColor] = useState('#000000');
  const [showLogo, setShowLogo] = useState(true);
  const [logoStyle, setLogoStyle] = useState('square'); // 'square' | 'circle'
  const [qrFontFamily, setQrFontFamily] = useState('helvetica'); // standard / google
  ...
  // Saving & Downloading states
  const [saving, setSaving] = useState(false);
  const [saveSuccess, setSaveSuccess] = useState(false);
  const [saveError, setSaveError] = useState('');
  const [downloadingPDF, setDownloadingPDF] = useState(false);

  // Load Google Fonts into the DOM head dynamically for real-time CSS previewing
  useEffect(() => {
  ...
  // Translate custom font option into visual inline CSS styling
    ...
    async function fetchProfile() {
      try {
        ...
        const { data, error } = await supabase
          .from('restaurants')
          .select('slug, name, theme_color, logo_url, wifi_ssid, wifi_password, wifi_encryption, qr_fg_color, qr_fg_color2, qr_bg_color, qr_style_type, qr_gradient_direction, qr_dot_style, qr_eye_style, qr_eye_color, qr_show_logo, qr_logo_style, qr_font_family')
          .eq('owner_id', user.id)
          .single();
        ...
        if (data) {
          setProfile(data);
          const fullUrl = `${window.location.origin}/menu/${data.slug}`;
          setMenuUrl(fullUrl);
          
          // Load styling choices from DB
          if (data.qr_fg_color) setFgColor(data.qr_fg_color);
          if (data.qr_fg_color2) setFgColor2(data.qr_fg_color2);
          if (data.qr_bg_color) setBgColor(data.qr_bg_color);
          if (data.qr_style_type) setQrStyleType(data.qr_style_type);
          if (data.qr_gradient_direction) setGradientDirection(data.qr_gradient_direction);
          if (data.qr_dot_style) setDotStyle(data.qr_dot_style);
          if (data.qr_eye_style) setEyeStyle(data.qr_eye_style);
          if (data.qr_eye_color) setEyeColor(data.qr_eye_color);
          if (data.qr_show_logo !== undefined && data.qr_show_logo !== null) setShowLogo(data.qr_show_logo);
          if (data.qr_logo_style) setLogoStyle(data.qr_logo_style);
          if (data.qr_font_family) setQrFontFamily(data.qr_font_family);
        }
      } catch (error) {
  ...
  // Regenerate Menu QR Code data URL whenever parameters change
  useEffect(() => {
    if (!menuUrl) return;
  ...
  // Generate Wi-Fi QR Code data URL whenever parameters change
  useEffect(() => {
    ...
    const generateWifiQR = async () => {
      try {
        ...
        const url = await drawCustomQR(wifiString, {
          ...
          eyeStyle,
          eyeColor,
          showLogo: false, // Wi-Fi scan-to-connect works best without center logo to prevent scanning delays
          logoUrl: null,
          logoStyle
        });
        setWifiQrUrl(url);
  ...
  // Generate and download a foldable double-sided A4 table tent card PDF containing Menu QR and Wi-Fi QR
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const midY = pageHeight / 2; // 148.5 mm

      // Helper to convert brand color hex to rgb
      const brandColorHex = profile?.theme_color || '#f97316';
      const brandRgb = hexToRgb(brandColorHex);

      // Load custom Google Font TTF file dynamically in jsPDF if selected
      const selected = AVAILABLE_FONTS.find(f => f.id === qrFontFamily);
      ...
      // 1. Rotate Menu QR code 180 degrees for Side A (Top Half)
      ...
      // ----------------------------------------------------
      // SIDE B (Bottom Half) - Right-side up (0 degrees)
      // ----------------------------------------------------
      // Draw outer border (accent brand color)
      doc.setLineWidth(1);
      doc.setDrawColor(brandRgb.r, brandRgb.g, brandRgb.b);
      ...
      if (hasWifi) {
        ...
        // Fallback: Menu QR on both sides if Wi-Fi not configured
        doc.setFont(pdfFont, 'bold');
        doc.setFontSize(20);
        doc.setTextColor(15, 23, 42);
          ...
          <div>
            ...
            <div className="space-y-5 mt-4 max-h-[60vh] overflow-y-auto pr-1 no-scrollbar font-sans">
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-[11px] text-slate-500 mb-1 font-medium font-sans">Background Color</label>
                  <div className="flex items-center space-x-2">
                    <input
                      type="color"
                      value={bgColor}
              ...
              <div className="space-y-3 pt-3 border-t border-slate-100">
                ...
                <div>
                  <label className="block text-[11px] text-slate-500 mb-1.5 font-medium">Dot Style</label>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      type="button"
              ...
              <div className="space-y-3 pt-3 border-t border-slate-100">
                ...
                <div>
                  <label className="block text-[11px] text-slate-500 mb-1.5 font-medium">Eye Style</label>
                ...
                <div>
                  <label className="block text-[11px] text-slate-500 mb-1 font-medium font-sans">Eye Frame Color</label>
              ...
              <div className="space-y-3 pt-3 border-t border-slate-100">
                ...
                {showLogo && (
                  <div>
                    <label className="block text-[11px] text-slate-500 mb-1.5 font-medium">Logo Shield Style</label>
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        type="button"
              ...
              <div className="space-y-3 pt-3 border-t border-slate-100 font-sans">
                ...
                <div>
                  <label className="block text-[11px] text-slate-500 mb-1.5 font-medium">Text Font Style</label>
                  <select
                    value={qrFontFamily}
                    onChange={(e) => setQrFontFamily(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-650 focus:outline-none"
                  >
              ...
              {profile?.wifi_ssid ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center py-6 text-center w-full font-sans">
                  ...
                  <a
                    ...
                    className="text-xs font-bold text-orange-600 hover:text-orange-700 border border-orange-200 bg-orange-50/50 px-3 py-1.5 rounded-lg transition-all"
                  >
